...
namespace VueAdmin.Web.Controllers
{
    ...
    public class UploadController : AdminBaseController
    {
        ...
        public async Task<IActionResult> SSIUploadImage()
        {
            ...
            var fileUrl = imgUploadPath + fileName;//返回文件的相对路径

            using (var stream = file.OpenReadStream())
            {
                //var isOk = await UploadAliyun(stream, alioss.UploadPath, fileName);
                var isOk = await Save(stream, imgUploadPath, fileName);
                if (isOk)
                {
                    ...
                    await _pictureRepository.AddAsync(dto);
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> UploadImageFromBase64(string strImgbase64, string md5, string oldFileName = "")
        {
            ...
            using (var stream = CreatetMemory(strImgbase64))
            {
                //var isOk = await UploadAliyun(stream, alioss.UploadPath, fileName);
                var isOk = await Save(stream, imgUploadPath, fileName);
                if (isOk)
        ...
        ///// <returns></returns>
        //private async Task<bool> UploadAliyun(Stream steam, string path, string fileName, bool isSaveLocal = false)
        //{
        //    try
        //    {
        //        // 初始化OssClient
        //        var client = new OssClient(alioss.Endpoint, alioss.AccessKeyID, alioss.AccessKeySecret);

        //        string md5 = OssUtils.ComputeContentMd5(steam, steam.Length);
        //        var objectMeta = new ObjectMetadata
        //        {
        //            ContentMd5 = md5,
        //        };
        //        client.PutObject(alioss.BucketName, path + fileName, steam, objectMeta);

        //        if (isSaveLocal)
        //        {
        //            await Save(steam, path, fileName);
        //        }
        ...
        /// <returns></returns>
        private MemoryStream CreatetMemory(string strBase64)
        {
            try
            {
            ...
            {
                Log.Error("生成MemoryStream流异常", ex);
                throw ex;
            }
        }
                    ...
                    if (await Save(stream, tmp, index.ToString()))
                    {
                        ...
                        if (total == index)
                        {
                            ...
                            if (mergeOk)
                            {
                                ...
                                await _attachRepository.AddAsync(dto);
                            }
                        }
